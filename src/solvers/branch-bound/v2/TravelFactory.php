...
class TravelFactoryBBV2
{
  public function create(array $route, int $newCity, array $distances, float $previousWeight, float $jumpWeight) : TravelBBV2
  {
    ...
    return new TravelBBV2($route, $newCity, $travels, $distances, $weight);
  }
  ...
  protected function minRow(&$distances) : array
    ...
    for ($i = 0; $i < count($distances); $i++)
    {
      $distanceRow = $distances[$i];
      for ($j = 0; $j < count($distances); $j++)
      {
        if ($distanceRow[$j] < $min[$i])
        {
					$min[$i] = $distanceRow[$j];
        }
      }
    ...
    for ($i = 0; $i < count($distances); $i++)
    {
      ...
      for ($j = 0; $j < count($distances); $j++)
      {
        if ($distanceRow[$j] !== INF && $min[$i] !== INF)
        {
					$distances[$i][$j] -= $min[$i];
        }
      }
    ...
    for ($i = 0; $i < count($distances); $i++)
    {
      $distancesRow = $distances[$i];
      for ($j = 0; $j < count($distances); $j++)
      {
        if ($distancesRow[$j] < $min[$j])
        {
					$min[$j] = $distancesRow[$j];
        }
      }
    }
    ...
    for ($i = 0; $i < count($distances); $i++)
    {
      ...
      for ($j = 0; $j < count($distances); $j++)
      {
        if ($distancesRow[$j] !== INF && $min[$j] !== INF)
        {
          $distances[$i][$j] -= $min[$j];
        }
      }
